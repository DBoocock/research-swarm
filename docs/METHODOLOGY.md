...
**Thinking mode and mandate stability.** Extended thinking is permitted only for synthesis and meta-agent calls — roles that (a) do not generate agent-attributed, persona-specific content and (b) do not touch agent mandates. Generation, debate, reflection, and all other agent-attributed roles use non-thinking mode. The reason is twofold: thinking traces can bleed disciplinary framing between calls in ways that are hard to audit, and enabling thinking for mandate-generation or agent-attributed generation calls risks subtle mandate drift that violates the stability invariant above. This is a hard constraint in the code (`isThinkingRole()` in `src/models.js`), not a soft preference.
