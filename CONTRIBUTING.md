...
- If your feature produces information that will be needed later — which agents contributed to a result, which round something happened in, which pair keys are relevant — record it as structured data at the time it is produced, not as something to be extracted from text later. The research map's `agents` and `debateRefs` fields are the canonical example: attribution is recorded at synthesis time, when the model has just processed the relevant context, rather than reconstructed afterward.
